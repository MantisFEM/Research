...
!!! warning "Single-component spaces need a specialised implementation."
    The default implementation only works for multi-component spaces, where the default is
    to use the component spaces.
...
function get_local_basis(
    ...
    throw(
        ArgumentError(
            "'get_local_basis' not implemented for single-component spaces of type $(typeof(space)).",
        ),
    )
end
...
function evaluate(
    ...
    basis_indices = get_basis_indices(space, element_id)
    println("Basis indices general: $basis_indices")
...
include("OtherSpaces/GenericC0Spaces.jl")
include("OtherSpaces/GenericBrokenSpaces.jl")

include("UnivariateSplines/UnivariateSplines.jl")
include("TensorProductSpaces/TensorProductSpaces.jl")

include("UnstructuredSpaces/ApproximateC1.jl")
