...
"""
    get_support_on_element(tp_space::TensorProductSpace{n, F1, F2}, element_id::Int) where {n, F1, F2}
...
# Returns
- `support::Vector{Int}`: Indices of the basis function supported on the element.
"""
function get_support_on_element(
    space::TensorProductSpace{n, F1, F2}, element_id::Int
) where {n, F1, F2}
    elems_per_dim = get_cart_num_elements(space)[element_id]
    support_F1 = get_support_on_element(get_constituent_spaces(space)[1], elems_per_dim[1])
    support_F2 = get_support_on_element(get_constituent_spaces(space)[2], elems_per_dim[2])

    num_basis_dim_1 = get_num_basis(get_constituent_spaces(space)[1])

    support = Vector{Int}(undef, length(support_F1) * length(support_F2))
    # Note that enumerate is safe here, support is a vector with 1-based indexing.
    for (idx, support_tup) in enumerate(Iterators.product(support_F1, support_F2))
        support[idx] = (support_tup[2] - 1) * num_basis_dim_1 + support_tup[1]
    end
...
function get_max_local_dim(space::TensorProductSpace)
    dim = 1
    for constituent_space in get_constituent_spaces(space)
        dim *= get_max_local_dim(constituent_space)
    end
    return dim#prod(get_max_local_dim, get_constituent_spaces(space))
end
