...
"""
struct AnalyticalFormField{manifold_dim, form_rank, G, E, extra_args} <:
       AbstractFormField{manifold_dim, form_rank, G}
    geometry::G
    expression::E
    label::String
    ...
    function AnalyticalFormField(
        form_rank::Int, expression::E, geometry::G, label::String, extra_args::Bool=false
    ) where {manifold_dim, E <: Function, G <: Geometry.AbstractGeometry{manifold_dim}}
        return new{manifold_dim, form_rank, G, E, extra_args}(geometry, expression, label)
    end

    function AnalyticalFormField(
        ::Val{form_rank}, expression::E, geometry::G, label::String, extra_args::Bool=false
    ) where {
        form_rank, manifold_dim, E <: Function, G <: Geometry.AbstractGeometry{manifold_dim}
    }
        return new{manifold_dim, form_rank, G, E, extra_args}(geometry, expression, label)
    end
end
...
"""
    get_form(form::FormField)

Returns the form space associated with the form field.

# Arguments
- `form::FormField`: The form field.

# Returns
- `<:AbstractFormSpace`: The form space associated with the form field.
"""
get_form(form::FormField) = form.form_space
...
function evaluate(
    ...
    form_basis_eval, form_basis_indices = evaluate(get_form_space(form), element_idx, xi)
    form_eval = Vector{Vector{Float64}}(undef, n_form_components)
    form_field_coefficients = get_coefficients(form)
...
function evaluate(
    ...
    return _evaluate(form, element_id, xi)
end

function _evaluate(
    form_field::AnalyticalFormField{manifold_dim, 0, G, E, false},
    element_idx::Int,
    xi::Points.AbstractPoints{manifold_dim},
) where {manifold_dim, G, E}
...
function _evaluate(
    form_field::AnalyticalFormField{manifold_dim, 0, G, E, true},
    element_idx::Int,
    xi::Points.AbstractPoints{manifold_dim},
) where {manifold_dim, G, E}
    x = Geometry.evaluate(get_geometry(form_field), element_idx, xi)
    form_eval = get_expression(form_field)(x, element_idx, xi)
...
function _evaluate(
    form::AnalyticalFormField{manifold_dim, 1},
    element_idx::Int,
    xi::Points.AbstractPoints{manifold_dim},
) where {manifold_dim}
    ...
    x = Geometry.evaluate(geometry, element_idx, xi)
    J = Geometry.jacobian(geometry, element_idx, xi)  # Jₖⱼ = ∂Φᵏ\∂ξⱼ
    form_eval = get_expression(form)(x) # size: num_points x image_dim
