...
function set_dirichlet_and_neumann_boundary_conditions(
    form::AbstractFormSpace, value::Float64
)
    return Dict{Int, Float64}(i => value for i in trace_normalder_basis_idxs(form))
end

function set_dirichlet_and_neumann_boundary_conditions(
    form::AbstractFormSpace, value::Vector{Float64}
)
    return Dict{Int, Float64}(i => value[i] for i in trace_normalder_basis_idxs(form))
end
...
function trace_basis_idxs(
    ...
    basis_idxs = [
        ...
        j in setdiff(1:num_sides, Int((num_sides + 1) / 2)) for i in dof_partition[k][j]  # TODO: is this general enough for multipatch?
    ]
    return basis_idxs
end

function trace_normalder_basis_idxs(
    form::AbstractForm{manifold_dim, form_rank, expression_rank, G}
) where {manifold_dim, form_rank, expression_rank, G}
    if FunctionSpaces.get_num_patches(get_fe_space(form)) > 1
        # This will require topological information to know which interfaces are outer
        ...
        throw(
            ArgumentError(
                "trace_normalder_basis_idxs not implemented for multipatch geometries"
            ),
        )
    end
    all_dofs = 1:get_num_basis(form)
    dofs_per_dim = FunctionSpaces.get_constituent_num_basis(get_fe_space(form))
    interior_dofs = reshape(all_dofs, dofs_per_dim)[3:(end - 2), 3:(end - 2)]
    return setdiff(all_dofs, interior_dofs)
end

function trace_normalder_basis_idxs(
    form::FormSpace{manifold_dim, form_rank, G, F}
) where {manifold_dim, form_rank, G, F <: FunctionSpaces.ApproximateC1Space}
    dofs = FunctionSpaces._get_dirichlet_and_neumann_dofs(get_fe_space(form))
    return dofs
end
