...
"""
function evaluate(
    form::Hodge{manifold_dim}, element_id::Int, xi::Points.AbstractPoints{manifold_dim}
) where {manifold_dim}
    return _evaluate_hodge(get_form(form), element_id, xi)
end
...
function _evaluate_hodge(
    form_expression::Trace{1, 1, expression_rank, G},
    element_id::Int,
    xi::Points.AbstractPoints{1},
) where {expression_rank, G}
    # _, sqrt_g = Geometry.metric(get_geometry(form_expression), interior_element, xi)
    _, sqrt_g = Geometry.metric(form_expression.trace_geometry, element_id, xi)
    form_eval, form_indices = evaluate(form_expression, element_id, xi)

    mat_size = size(form_eval[1])
    hodge_eval = [Matrix{Float64}(undef, mat_size)]
    # ⋆α₁ⁿdξ₁∧…∧dξₙ = α₁ⁿ(√det(gᵢⱼ))⁻¹.
    for id in CartesianIndices(mat_size)
        hodge_eval[1][id] = form_eval[1][id] * (1 / sqrt_g[id[1]])
...
#     inds1 = zeros(Int, 2)
#     inds2 = zeros(Int, 2)
#     for interior_patch_id in
#         1:Geometry.get_num_patches(get_geometry(get_form(form_expression)))
#         interior_element = get_interior_element(
#             form_expression, element_id, interior_patch_id
#         )
#         if interior_patch_id == 1
#             edge = form_expression.edge
#         else
#             edge = form_expression.edge + 2
#         end
#         interior_xi = create_interior_xi(xi, 1, 2, edge)
#         # _, sqrt_g = Geometry.metric(get_geometry(form_expression), interior_element, xi)
#         _, sqrt_g = Geometry.metric(form_expression.trace_geometry, element_id, xi)
#         form_eval, form_indices = evaluate(
#             get_form(form_expression), interior_element, interior_xi
#         )
#         # Because we restrict ourselves to expression with rank 0 or 1, we can
#         # extract the first set of indices (only set), for higher ranks, we need
#         # to change this
