...
- `quad_rule::Q`: The quadrature rule used for the integral.

# Type Parameters
- `manifold_dim::Int`: The dimension of the manifold.
- `F`: The type of the form expression.
- `Q`: The type of the quadrature rule.
# Inner Constructors
...
struct Integral{manifold_dim, F, Q} <: AbstractRealValuedOperator{manifold_dim}
    ...
    function Integral(
        ...
        if Geometry.get_num_elements(geom) != Quadrature.get_num_base_elements(quad_rule)
            @warn """The number of elements in the geometry and quadrature rule must \
...
"""
function evaluate(
    form::Integral{manifold_dim, F, Q}, global_element_id::Int
) where {
    manifold_dim,
    form_rank,
    expression_rank,
    ...
    quad_rule = get_quadrature_rule(form)
