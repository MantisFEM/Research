...
"""
    zero_form_biharmonic(
        inputs::AbstractInputs, dΩ::Quadrature.AbstractGlobalQuadratureRule
    )

Function for assembling the weak form of the 0-form biharmonic problem.
...
"""
    solve_zero_form_biharmonic(X⁰, fₑ, dΩ)
...
"""
function solve_zero_form_biharmonic(X⁰, fₑ, dΩ)
    weak_form_inputs = WeakFormInputs(X⁰, fₑ)
    lhs_expressions, rhs_expressions = zero_form_biharmonic(weak_form_inputs, dΩ)
    weak_form = WeakForm(lhs_expressions, rhs_expressions, weak_form_inputs)
    # homogeneous boundary conditions on value and normal derivative.
    bc = Forms.set_dirichlet_and_neumann_boundary_conditions(X⁰, 0.0)
