...
struct WeakFormInputs{manifold_dim, TeF, TrF, F} <: AbstractInputs
    ...
    function WeakFormInputs(forms::TrF, forcings::F) where {TrF, F}
        @warn "This is a shady WeakFormInputs!"
        return new{Forms.get_manifold_dim(forms[1]), TrF, TrF, F}(forms, forms, forcings)
    end
