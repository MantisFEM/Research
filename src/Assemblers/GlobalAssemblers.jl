...
function assemble(
    ...
    for row in 1:num_rows
        for col in 1:lhs_num_cols
            if !(typeof(lhs_expressions[row][col]) == Int)
                for elem_id in 1:Forms.get_num_elements(lhs_expressions[row][col])
        ...
        for col in 1:rhs_num_cols
            if !(typeof(rhs_expressions[row][col]) == Int)
                for elem_id in 1:Forms.get_num_elements(rhs_expressions[row][col])
...
function build_matrix(
    ...
    matrix = zeros(Float64, size[1])
    for i in eachindex(rows, vals)
        matrix[rows[i]] += vals[i]
    end
