...
"""
    gauss_jacobi(N::Integer)
"""
function gauss_jacobi(N::Integer, alpha, beta)

    ξ, w = FastGaussQuadrature.gaussjacobi(N, alpha, beta)
    @. ξ = (ξ + 1.0) / 2.0
    # @show w
    # @show alpha
    # @show beta
    @. w = w * 0.5^(alpha+beta+1)# * 2^(-beta) 0.5 *
    # @show w
    nodes = Points.CartesianPoints((ξ,))

    return CanonicalQuadratureRule(nodes, w, "Gauss-Jacobi")
end
