...
include("examples/Geometry/Pentagon.jl")

# Options:
# "AP_TH_p_r_n":
# WC0 = S^p_{r}, G = [S^p_{r-1} S^p_{r-1}, S^p_{r-1} S^p_{r-1}], Q = S^{p-1}_{r-1}
const case_pre = "AP_TH_2_1_5_pentagon"
const case = "AP_TH_3_2_5_pentagon"
const case_post = "AP_TH_4_3_5_pentagon"
...
function exact_sol_func(x::Matrix{Float64})
    return [@. sin(pi * x[:, 1] + pi/3) * sin(pi * x[:, 2] + pi/5)]
end
function exact_dsol_func(xx::Matrix{Float64})
    x = xx[:, 1]
    y = xx[:, 2]
    return [
        pi .* cos.(pi .* x .+ pi ./ 3) .* sin.(pi .* y .+ pi ./ 5),
        pi .* sin.(pi .* x .+ pi ./ 3) .* cos.(pi .* y .+ pi ./ 5),
    ]
end
function exact_ddsol_func(xx::Matrix{Float64}) # laplacian
    ...
    y = xx[:, 2]
    return [
        - 2 .* pi .^2 .* sin.(pi .* x .+ pi ./ 3) .* sin.(pi .* y .+ pi ./ 5)
    ]
end
function forcing_function(xx::Matrix{Float64})
    ...
    y = xx[:, 2]
    return [
        4 .* pi .^4 .* sin.(pi .* x .+ pi ./ 3) .* sin.(pi .* y .+ pi ./ 5)
    ]
end
...
function run_case(case)
    ...
    for num_elements in num_elements_study
        ...
        # Create geometry
        geometry_i = create_pentagon_geometry(num_elements)
    ...
    pfilename = "TestCase3-nels$(num_elements_study)-p$(p)-r$(r)"
    if save_csv
        save_to_csv(
            pfilename,
            hs,
